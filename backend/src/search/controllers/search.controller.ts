...
export class SearchController {
  ...
  @Get()
  @ApiOperation({
    summary: 'Global search across tasks, workspaces, and users',
  })
  @ApiQuery({
    name: 'q',
    required: true,
  ...
  @ApiQuery({
    name: 'types',
    required: false,
    description: 'Comma-separated types: tasks,workspaces,users',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  async searchTasks(
    ...
    const tasks = await this.searchService.searchTasks(
      userId,
      query.q,
      filters,
    );
